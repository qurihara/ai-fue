 ...
 *
 * [* そろえて開くものは、合成後の秘密で1つの口座にする。] ハートの2枚と北斎のタイルは、
 * 2枚そろえば同じ秘密が戻るので、口座も1つである。どの2枚を選んでも同じ口座が開く。
 * 一方 2-of-3 の3つは、断片ごとに別の口座を持たせてある（1つでも吹けば何かが開く、
 * という見せ方のため）。
 *
 * [* 北斎のタイルは、断片ごとの口座を作っていない。] 実演の録画から読めた記号のうち
 * 誤り訂正が働いた3枚（6・7・8）は、訂正の結果が実物と食い違っていた。信用できるのは
 * 2・3・4・9の4枚で、それらの組から戻る秘密 314159 だけが確かである。
 */
const WALLETS = [
  ...
  ["ハートのカード2枚",         [1, 6, 8, 9, 8, 7]],
  ["北斎タイル（どの2枚でも）", [1, 10, 5, 0, 3, 10]],
];
